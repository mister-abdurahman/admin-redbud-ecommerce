...
function BrandForm({ BrandToEdit = {}, closeModal, onCloseModal }: any) {
  ...
  const { EditBrand, isEditing } = useEditBrand();

  const isWorking = isCreating || isEditing;

  function handleForm(data: IBrand) {
    if (editId)
      EditBrand(
        {
          newBrand: data,
      ...
      CreateBrand(
        { newBrand: { name: data.name, description: data.description } },
        {
          onSuccess: (data) => {
            reset();
            onCloseModal?.();
